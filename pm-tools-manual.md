...
- **Manually adjusting a division's Estimate figure** — admins only, and only in admin mode, for every division including the sub-groups that don't carry a cost code of their own (Jobsite Dumpster/Porta John and Jobsite Parking, under 5121). Click the figure in a gray division row and a confirmation asks "You're about to manually adjust an estimate figure. Do you want to proceed?" before anything happens. Say yes and it becomes an editable field, pre-filled with the current number (blank if there's never been one); save it like any other figure. This is meant to be rare — a manual override is temporary in the sense that **Back to Estimate Spreadsheet and Tab** will overwrite it again from the estimate file, same as any other estimate figure (except for a sub-group with no cost code of its own, which the estimate file was never going to touch anyway).
...
- **Going more than 10% over budget gets flagged automatically.** Estimate lives at the division level, not per line — so this is checked at that level: if editing a line's Budget figure would push its division's Budget total more than 10% over that division's Estimate total, that line's comment box opens on its own with a Budgeting comment already written for you (how far over it puts the division). Nothing to type — click **Post** (or hit Enter) and the note and the figure save together. You can edit the pre-filled text first if you want to add detail, or hit Esc to back out and leave the amount as it was. Once a line has any Budgeting comment on it, later edits to that same line aren't flagged again.
